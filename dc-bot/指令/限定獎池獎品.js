...
function createPrizeReply(interaction, 獎品清單, content, ephemeral = false, filterRarity = null) {
  const embed = new EmbedBuilder().setTitle('🎯 當前限定獎池獎品').setColor(0x3399FF);
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const 獎品清單 = serverData.限定獎池設定.獎品清單;
    ...
    if (opt.操作 === null) {
      if (!獎品清單.length)
        return interaction.reply({ content: '尚未設定任何限定獎池獎品。', ephemeral: true });

      const reply = createPrizeReply(interaction, 獎品清單, '📋 以下是目前限定獎池獎品：', true, opt.稀有度);
    ...
    // 刪除
      ...
      serverData.限定獎池設定.獎品清單 = 獎品清單.filter(i => i.名稱 !== opt.名稱);
    ...
    if (serverData.限定獎池設定.獎品清單.length) {
      const reply = createPrizeReply(interaction, serverData.限定獎池設定.獎品清單, '📋 操作完成，以下是目前限定獎池獎品：', false, opt.稀有度);
